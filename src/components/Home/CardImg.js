...
function CardImg({ cardImg, nickname }) {
    ...
    const filteredLetters = letters.filter(letter => letter.name === cardImg).map(letter => {
    ...
    return (
        <>
            ...
            {isPopupVisible && <Popup isVisible={true} onClose={handleClosePopup} nickname={nickname} letter={filteredLetters[0]} />}
        </>
    );
